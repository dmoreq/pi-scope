...
// ── Defaults ──────────────────────────────────────────────────────────────
export const DEFAULT_PROVIDER_GUIDANCE_OPTIONS = {
    enabled: true,
};
...
function loadGuidanceConfig(agentDir) {
...
function getCandidateFiles(modelId, provider, config) {
 ...
 */
function shouldLoad(dir, filename) {
    ...
    if (agentsExists) {
        try {
            ...
            if (agentsContent === candidateContent)
                return false;
        }
        catch {
...
// ── Directory scan ────────────────────────────────────────────────────────
function getDirectories(cwd, agentDir) {
    const dirs = [];
    const seen = new Set();
 ...
 */
export function loadProviderGuidance(cwd, provider, modelId) {
 ...
 */
export function formatProviderGuidanceSection(files) {
    if (files.length === 0)
        return '';
    const body = files
        .map((file) => `## ${file.path}\n\n${file.content}`)
 ...
 */
export function buildGuidanceNotification(files, cwd) {
    if (files.length === 0)
        return '';
    const paths = files
        .map((f) => {
