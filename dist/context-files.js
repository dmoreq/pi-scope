 ...
 */
export function loadContextFiles(cwd, options = DEFAULT_CONTEXT_FILE_OPTIONS) {
 ...
 * injection.
 */
export function formatContextSection(files, options = DEFAULT_CONTEXT_FILE_OPTIONS) {
    if (files.length === 0)
        return '';
 ...
 */
export function formatDisplayPath(filePath, cwd) {
 ...
 */
export function buildStartupNotification(files, cwd, options = DEFAULT_CONTEXT_FILE_OPTIONS) {
    if (files.length === 0)
        return '';
    const sectionTitle = options.sectionTitle ?? DEFAULT_CONTEXT_FILE_OPTIONS.sectionTitle;
    const paths = files
