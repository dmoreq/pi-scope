...
function resolvePath(raw, projectRoot) {
    ...
    if (projectRoot)
        return normalize(resolve(projectRoot, raw));
    return normalize(raw);
}
function pathExists(raw, projectRoot) {
    try {
...
function hasValidExtension(p, extensions) {
 ...
 */
function buildPathRegex(extensions) {
 ...
 */
function buildCitationRegex(extensions) {
    const exts = extensions.map(e => e.slice(1)).join('|');
    const dir = "(?:[./\\w@_-]+/)*";
    const file = "[\\w@_-]+\\.(?:";
...
// ── Text scanning ─────────────────────────────────────────────────────────
export function detectPathsInText(text, options = {}) {
    if (!text)
        return [];
    ...
    for (const match of text.matchAll(citeRe)) {
        ...
        if (!raw || raw.length < 3 || seen.has(raw))
            continue;
        seen.add(raw);
        if (validate && !pathExists(raw, projectRoot))
            continue;
        results.push({
...
export function detectPathsInToolCall(toolName, input, options = {}) {
    if (!input || !FILE_TOOLS.has(toolName))
        return [];
    const extensions = options.extensions ?? DEFAULT_EXTENSIONS;
    const projectRoot = options.projectRoot;
    const results = [];
...
export function detectPathsInOutput(_toolName, content, options = {}) {
    if (!content)
        return [];
...
export function detectPathsInMessage(message, options = {}) {
    const results = [];
    if (message.role === 'user' || message.role === 'assistant') {
        const text = typeof message.content === 'string'
            ...
            : Array.isArray(message.content)
                ...
                : '';
        if (text)
            results.push(...detectPathsInText(text, options));
    }
