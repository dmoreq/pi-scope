...
const _tool = (content: string): ContextMessage => ({ role: 'tool', content })
...
describe('deduplicate', () => {
  it('removes identical consecutive user messages', () => {
    const messages = [user('hello'), assistant('hi there'), user('hello'), assistant('hi there')]
  ...
  it('preserves unique user/assistant messages', () => {
    const result = deduplicate([user('first'), assistant('r1'), user('second'), assistant('r2')])
    expect(result).toHaveLength(4)
  })

  it('preserves tool messages even if identical', () => {
    const result = deduplicate([user('hello'), toolResult('{"result": "ok"}'), toolResult('{"result": "ok"}')])
    expect(result).toHaveLength(3)
  })

  it('deduplicates across non-consecutive messages', () => {
    const result = deduplicate([user('hello'), assistant('r1'), user('hello'), assistant('r2')])
    expect(result).toHaveLength(3)
  })

  it('handles empty array', () => expect(deduplicate([])).toEqual([]))
  it('handles single message', () => expect(deduplicate([user('test')])).toHaveLength(1))

  it('does not dedup non-user/assistant roles', () => {
    // Messages with undefined role are NOT deduped (only user/assistant)
    const result = deduplicate([{ content: 'a' }, { content: 'a' }] as ContextMessage[])
...
describe('supersedeWrites', () => {
  ...
  it('preserves writes for different files', () => {
    const result = supersedeWrites([toolResult('{"path": "a.ts"}'), toolResult('{"path": "b.ts"}')])
    expect(result).toHaveLength(2)
  })
...
describe('purgeErrors', () => {
  ...
  it('preserves error not followed by success', () => {
    const result = purgeErrors([toolResult('{"isError": true}'), user('still broken')])
    expect(result).toHaveLength(2)
  })

  it('removes error using status field', () => {
    const result = purgeErrors([toolResult('{"status": "error"}'), toolResult('{"status": "ok"}')])
  ...
  it('removes consecutive errors when first is followed by success (skipping second error)', () => {
    ...
    expect(result).toHaveLength(2)
    expect(result.map(m => m.content)).toEqual(['{"isError": true, "m": "fail1"}', '{"isError": false, "m": "ok"}'])
  })
...
describe('applyPruningRules', () => {
  it('applies all enabled rules', () => {
    const messages = [user('hello'), user('hello'), toolResult('{"isError": true}'), toolResult('{"status": "ok"}')]
  ...
  it('returns accurate removed count', () => {
    const { pruned, removed } = applyPruningRules([user('a'), user('a'), user('b'), user('b')])
  ...
  it('can disable specific rules via config', () => {
    const { removed } = applyPruningRules(
      [user('hello'), user('hello'), toolResult('{"isError": true}'), toolResult('{"status": "ok"}')],
